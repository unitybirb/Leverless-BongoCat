...
const VERSION = '1.1.0';  // Added color picker feature
...
function restoreBackgroundColor(): void {
    const savedColor = localStorage.getItem('bongo-cat-bg-color');
    if (savedColor) {
...
function initializeColorPicker(): void {
    ...
    // Load saved color
    const savedColor = localStorage.getItem('bongo-cat-bg-color') || '#000000';
...
function updateButtonOverlays(gp: Gamepad): void {
  ...
  let leftState = 0;
  if (x === -1 && y === 0) leftState = 1;
  else if (x === 0 && y === -1) leftState = 2;
  else if (x === 1 && y === 0) leftState = 3;
  else if (x === 0 && y === 1) leftState = 4;
  ...
  br.forEach((el, idx) => {
    if (el) el.classList.toggle('invisible', idx !== armIndex);
  });
  if (rightUp) {
